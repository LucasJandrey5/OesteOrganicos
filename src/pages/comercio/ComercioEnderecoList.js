...
export default function ComercioEnderecoList({ navigation,route }) {
  const {item}=route.params
  const [data, setData] = useState({
    id:item.id, 
  });
  console.log("valores vindo para enderecos comercio")
  console.log(item);

  const loadData = async () => {
    try {
      const response = await API.call('comercio-endereco/'+data.id);
  ...
  return (
    ...
    <ScrollView contentContainerStyle={{ width: '100%' }} marginTop="15px" >
        ...
        <List 
          ...
          {data?.enderecos?.map((item, index) => (
            ...
            <List.Item
              key={index}
              onPress={() => {
                navigation.navigate('ComercioEnderecoForm', { item: item || null });
      ...
      <Fab
        ...
        onPress={() => navigation.navigate('ComercioEnderecoForm', { item: item.id|| "" })}
      />
    </>
  );
